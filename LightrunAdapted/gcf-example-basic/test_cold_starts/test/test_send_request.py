...
class TestSendRequestTask(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.url = 'https://test-function-001-abc123.run.app'
        self.function_index = 1
    
    def test_init(self):
        """Test SendRequestTask initialization."""
        task = SendRequestTask(self.url, self.function_index)
        
        self.assertEqual(task.url, self.url)
    ...
    def test_execute_successful_request(self, mock_time, mock_get):
        ...
        self.assertEqual(result['_function_index'], self.function_index)
        self.assertEqual(result['_url'], self.url)
        self.assertEqual(result['isColdStart'], True)
        self.assertEqual(result['totalDuration'], '1000000000')
    ...
    def test_execute_http_error(self, mock_time, mock_get):
        ...
        self.assertEqual(result['function_index'], self.function_index)
